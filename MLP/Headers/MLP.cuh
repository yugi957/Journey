...
public:
	...
	void forward_conv(float* x);
	vector<float> getForwardConv(float* d_x);
	...
	float backward_conv(float* d_x, float* d_y);
	...
	float** d_conv_weights_href;
	float** d_gradient_href;
	float** d_conv_gradient_href;
